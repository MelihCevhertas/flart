...
**v1.14 (19 Mayıs 2026):** v0.2.0 release based on FollowUp user feedback. İki ana tema: (1) `flart savings` default scope kullanıcı beklentisiyle uyumsuzdu (multi-project agent session sonrası kullanıcı "şu anki proje" rakamını istiyor, "tüm projeler kümülatifi"ni değil) — default behavior CWD-scoped'a değiştirildi, eski davranış `--all` flag'iyle korundu, `--project-path=<path>` explicit override eklendi, mevcut boolean `--project` deprecation cycle ile bir versiyon daha yaşar (warning + CWD-equivalent davranış). CWD pubspec.yaml içermiyorsa graceful `--all` fallback + stderr note. (2) Sub-agent adoption gap: parent agent flart kullanıyor ama Task tool ile spawn ettiği sub-agent'lar default `flutter analyze`/`dart test`'e dönüyordu çünkü CLAUDE.md routing block sub-agent prompt'una merge edilmiyor. **`SubagentStart` event reddedildi:** Claude Code v2.1.116+ listesinde olsa da unstable (GH #27755 settings.json trigger bug, #19170 schema gaps); production-ready değil. **Çözüm: `PreToolUse` matcher `Task`** (v2.0.10+ stabil, dokumante). Parent Task tool çağırdığında hook tetiklenir, `hookSpecificOutput.additionalContext` ile flart usage hint (~150 token) sub-agent prompt'una merge olur. Hook ayrıca yeni `subagent_activations` tablosuna (migration v2) timestamp + project_path + parent_session_id yazar. Aktivasyon sayacı `flart savings` raporunda separate satır (text formatter + JSON `summary.subagent_activations`) olarak görünür; `invocations` tablosuyla karışmaz çünkü ölçülebilir byte/token tasarrufu yok. **Read/Grep hook reddedildi:** PreToolUse Read/Grep/Edit matcher'ları teknik olarak destekleniyor ama (a) Read truncation silent agent confusion yaratır — agent dosyayı tam gördüğünü varsayıp yanlış karar verir; (b) PostToolUse output'u replace edemez sadece feedback ekleyebilir; (c) token tasarrufu iteration cost'una nadiren değer. v0.3.0 backlog'a opt-in beta olarak girer, gerçek kullanıcı verisi gelince yeniden değerlendirilir. **Doc honesty:** README "Frequently misunderstood" subsection eklendi (Read/Grep exclusion rationale + sub-agent context cost transparency). CLAUDE.md template'ine bir satır eklendi: Task ile spawn edilen sub-agent'lar otomatik routing inherit ediyor. **Tag akışı:** v1.13'tekiyle aynı (push → tag v0.2.0 → release.yml → 2 binary build → draft → user publish). Migration v2 additive, mevcut v0.1.0 DB'leri otomatik upgrade olur, rollback path yok (additive migration policy).
