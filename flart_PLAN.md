...
**v1.15 (19 Mayıs 2026):** v0.3.0 release — Bash output mutation (Claude Code v2.1.121+) + v0.2.0'dan inline edilen iş (sub-agent context, CWD savings) tek paket olarak çıktı. **Tetikleyici:** Kullanıcı Claude Code'u v2.1.144'e güncellediği için `hookSpecificOutput.updatedToolOutput` (v2.1.121+) artık kullanılabilir; üç ayrı küçük release yerine paradigma değişikliği tek release olarak yayımlanır. **Yeni capability — PostToolUse / Bash output mutation:** `flart_hooks/lib/src/bash_post_filter.dart` (pure logic) + `flart_cli/lib/commands/bash_post_hook_command.dart` (stdin/stdout pipeline) + `bash_post_hook.sh` template. Decision tree: empty / `flart …` prefix / tee-read (cat/head/tail/less/more/wc/grep + arg in tee dir) / `FLART_FULL_OUTPUT=1` env / small (≤30 lines, exit 0) → passthrough; 31-200 lines → head 20 + tail 5 + tee path; >200 lines → head 15 + tail 5 + error grep (yalnız elided body üzerinden) + tee; exit ≠ 0 → framed `Command failed (exit N)` + stderr (2KB cap) + stdout last 20 + tee. Anti-bloat fallback (FilterRunner pattern): filteredBytes ≥ rawBytes → bypass. **Version detection:** `flart_hooks/lib/src/claude_version.dart` regex `(\d+)\.(\d+)\.(\d+)` ile `claude --version` parse'lar. Tolerant — null on parse failure, hook installer null'da PostToolUse'u skip eder. Threshold v2.1.121, sabit `ClaudeCodeVersion.outputMutationMinimum`. **Installer 3 entry yönetiyor:** PreToolUse/Bash (rewrite.sh), PreToolUse/Task (task_hook.sh), PostToolUse/Bash (bash_post_hook.sh — version-gated). Install eski Claude'da yumuşak skip + açıklayıcı mesaj. **Uninstall hep üçünü temizler**, downgrade sonrası stale entry kalmasın diye. **Reporter:** `bash_post` module otomatik `byModule` GROUP BY ile görünür — code change yok. Command kolonu: first token, special-case `python -c`/`dart -c`/shell builtin → "shell". **BashFilterThresholds constants (flart_core/src/constants.dart):** Hard-coded named constants; v0.4.0'da config'leme refactor'u. **Read/Grep/Edit/Write hâlâ excluded** — README'de "What flart deliberately doesn't intercept" subsection olarak ayrıntılı; output mutation Bash dışına genişletilmiyor çünkü silent context drops agent confusion yaratır, Grep mode'unu agent zaten bilinçli seçer, Edit/Write input agent'ın kendi draft'ı. **Tee recovery dual path:** Heuristik (cat/head/tail tee path'inde → bypass) + explicit (FLART_FULL_OUTPUT=1 env prefix → bypass) → her ikisi de CLAUDE.md routing'de belgelenir. **Manuel test paketi:** `V0.3.0_TEST_PLAN.md` yeni dosya, 7 senaryo (analyze, grep, tee recovery, FLART_FULL_OUTPUT escape, error preservation, version detection mock, savings by-module) + synthetic stdin smoke. Push öncesi kullanıcı tarafından koşturulur. **Tag akışı v1.14'tekiyle aynı** (push → tag v0.3.0 → release.yml → 2 binary build → draft → user publish). Migration v2 backward-compatible, v0.1.0 DB'leri auto-upgrade. v0.2.0 tag silinmişti (Plan v1.14); v0.3.0 doğrudan v0.1.0'dan sonra tek release.

**v1.14 (19 Mayıs 2026):** v0.2.0 release strateji geri çekildi, **iş v0.3.0'a inline ediliyor**. Tag v0.2.0 ve draft release silindi (commit `a485be4` main'de kalıyor — kod değişiklikleri v0.3.0'ın parçası). **Sebep:** Kullanıcının Claude Code v2.1.144'e güncellenmesi sonrası `PreToolUse` hook'ları için output mutation (v2.1.121+) feature artık kullanılabilir. Sub-agent context injection + CWD-scoped savings + Bash output mutation üç ayrı küçük release yerine **tek paradigma değişikliği** olarak v0.3.0 altında release edilecek (ayrı release note'ları "yeni hook ekledik, sonra output mutation ekledik" şeklinde fragmented görünür). Mevcut `a485be4` commit içeriği (Task hook, subagent_activations, `--all`/`--project-path` flags, `--project` deprecation, doc updates) v0.3.0 surface'ının yarısı — diğer yarısı output mutation. **Plan içeriği (v0.2.0 olarak yazılmıştı, şimdi v0.3.0 olarak okunur):** (1) `flart savings` default scope CWD-scoped'a değişti, eski davranış `--all` ile korundu, `--project-path=<path>` explicit override eklendi, boolean `--project` deprecation cycle ile bir versiyon daha yaşar. CWD pubspec.yaml içermiyorsa `--all` fallback + stderr note. (2) Sub-agent context injection — `SubagentStart` event reddedildi (Claude Code v2.1.116+ listesinde ama unstable, GH #27755 + #19170); **`PreToolUse` matcher `Task`** kullanıldı (v2.0.10+ stabil), `hookSpecificOutput.additionalContext` ile flart usage hint sub-agent prompt'una merge olur. `subagent_activations` tablosu (migration v2) timestamp + project_path + parent_session_id yazar, sayaç savings raporunda separate satır olarak görünür. (3) Read/Grep hook reddedildi — silent Read truncation agent confusion yaratır, PostToolUse output replace edemez. **v0.3.0 kalan iş:** Bash output mutation tasarımı + implementasyonu + dokümantasyonu. Tag akışı v0.3.0 release'ine ertelendi. **Manuel temizlik adımı kullanıcıda:** GitHub UI'da v0.2.0 draft release'i silinmeli (tag silinince orphan kaldı; public API draft'ları listelemez ama UI'da görünür).
